...
extern int		messages_disabled;
//extern int		show_stats;			// ADDED FOR PSP-STATS
...
*/
    ...
    if (showMessages || dont_show == 0)// || message_dontfuckwithme)		(ADDITION FOR PSP)
    {
	    ...
	    if(messages_disabled == 1)					//	(ADDITION FOR PSP)
		dont_show = 1;
        }
...
//    static int          num_nobrainers = 0;
    ...
    // FOR PSP: NEED MORE CLARIFICATION ON WHAT THIS DOES

    if(ev->type == ev_joystick && ev->data1 == 1 && data->btns_d)
    {
        return false;
    }
/*
    else if (ev->data1 == KEY_LEFTTRIGGER || ev->data1 == KEY_RIGHTTRIGGER)
    {
...
/*
    ...
    else
    {
        ...
        if (altdown)
        {
            ...
            HU_queueChatChar(KEY_START); // DEBUG!!!

            // send the macro message
            while (*macromessage)
                HU_queueChatChar(*macromessage++);
            HU_queueChatChar(KEY_START);
