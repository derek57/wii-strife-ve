...
/*
            y = viewwindowy+4;							// CHANGED FOR HIRES
        V_DrawPatch(viewwindowx + (scaledviewwidth - 68) / 2, y,		// CHANGED FOR HIRES
                          W_CacheLumpName (("M_PAUSE"), PU_CACHE));		// CHANGED FOR HIRES
*/
	    y = (viewwindowy >> hires)+4;					// CHANGED FOR HIRES
	V_DrawPatchDirect((viewwindowx >> hires)				// CHANGED FOR HIRES
		+ ((scaledviewwidth >> hires) - 68) / 2, y,			// CHANGED FOR HIRES
...
/*
    return (x >= 0 && y >= 0					// CHANGED FOR HIRES
            && x + SHORT(patch->width) <= SCREENWIDTH 		// CHANGED FOR HIRES
            && y + SHORT(patch->height) <= SCREENHEIGHT);	// CHANGED FOR HIRES
*/
    return (x >= 0 && y >= 0					// CHANGED FOR HIRES
	    && x + SHORT(patch->width) <= ORIGWIDTH		// CHANGED FOR HIRES
	    && y + SHORT(patch->height) <= ORIGHEIGHT);		// CHANGED FOR HIRES
}
...
//    V_DrawBlock(0, 41, 320, 95, rawgfx_startup0 + (320*57));		// CHANGED FOR HIRES
    V_DrawScaledBlock(0, 41, 320, 95, rawgfx_startup0 + (320*57));	// CHANGED FOR HIRES
}
...
//    V_DrawBlock(laserpos, 156, 16, 16, rawgfx_startlz[laserpos % 2]);		// CHANGED FOR HIRES
    V_DrawScaledBlock(laserpos, 156, 16, 16, rawgfx_startlz[laserpos % 2]);	// CHANGED FOR HIRES
...
//    V_DrawBlock(14, 138 + robotpos, 48, 48, rawgfx_startbot);			// CHANGED FOR HIRES
    V_DrawScaledBlock(14, 138 + robotpos, 48, 48, rawgfx_startbot);		// CHANGED FOR HIRES

    // Draw the peasant
    // Blitted 32 bytes for 64 rows starting at 699142
    // 699142 - 0xA0000 == 43782, which % 320 == 262, / 320 == 136
//    V_DrawBlock(262, 136, 32, 64, rawgfx_startp[laserpos % 4]);		// CHANGED FOR HIRES
    V_DrawScaledBlock(262, 136, 32, 64, rawgfx_startp[laserpos % 4]);		// CHANGED FOR HIRES
...
*/
	...
	printf("                  PLEASE COPY THE PWAD 'PSPSTRIFE.WAD' THAT CAME                ");
...
//	sceKernelDelayThread(5000*1000);
	...
	printf("\n\n\n\n\n");
	printf(" ===============================================================================");
	printf("             WARNING: RESOURCE PWAD FILE 'PSPSTRIFE.WAD' MISSING!!!             ");
...
//	sceKernelDelayThread(5000*1000);
    ...
    else if(fsize == 28372168)	// FILE SIZE OF STRIFE1.WAD FOR v1.0
    {
	STRIFE_1_0_SHAREWARE = false;
    ...
    else if(fsize == 28377364)	// FILE SIZE OF STRIFE1.WAD FOR v1.2, v1.3, v1.31
    {
	STRIFE_1_0_SHAREWARE = false;
	STRIFE_1_1_SHAREWARE = false;
	STRIFE_1_0_REGISTERED = false;
...
//	pspDebugScreenSetBackColor(0x000000);	// black
    ...
    if( (strife1_wad_exists == 1 && fsize != 28372168
				 && fsize != 28377364) ||
	(strife0_wad_exists == 1 && fsize != 9934413))
    {
	printf("\n\n\n\n\n");
	printf(" ===============================================================================");
	printf("        !!! STRIFE MAIN WAD FILE MISSING, NOT SELECTED OR WRONG IWAD !!!        ");
   	printf("                                                                                \n");
	printf("                                 QUITTING NOW ...                               ");
	printf(" ===============================================================================");
...
/*
    if(voices_wad_exists == 1 && fsizev != 27319149)
    {
...
//    if(devparm)
    {
    	...
    	else if(devparm || STRIFE_1_1_SHAREWARE)
	    printStyledText(1, 1,CONSOLE_FONT_GREEN,CONSOLE_FONT_BLACK,CONSOLE_FONT_BOLD,&stTexteLocation,"                       STRIFE: Quest for the Sigil v1.1                        ");
    	else if(devparm && STRIFE_1_X_REGISTERED)
	    printStyledText(1, 1,CONSOLE_FONT_GREEN,CONSOLE_FONT_BLACK,CONSOLE_FONT_BOLD,&stTexteLocation,"                      STRIFE:  Quest for the Sigil v1.31                       ");
    }
...
/*
    ...
    I_AtExit(M_SaveDefaults, false);	// DISABLED & MOVED TO I_QUIT FOR THE WII (SEEMS BUGGY HERE)
...
//	if (strife1_wad_exists == 1)
	{
	    if(fsize == 9934413)	// FILE SIZE OF STRIFE1.WAD FOR v1.0
	    {
		STRIFE_1_0_REGISTERED = false;
		STRIFE_1_X_REGISTERED = false;
		STRIFE_1_0_SHAREWARE = false;
...
//	    pspDebugScreenSetBackColor(0x000000);	// black
    	    ...
    	    if(devparm)
    	        DEH_printf(" Play Voices = %d\n", disable_voices == 0);
*/
    	    if(devparm || STRIFE_1_1_SHAREWARE) // [STRIFE] Devparm only
    	        DEH_printf("W_Init: Init WADfiles.\n");
	    ...
	    if (load_extra_wad == 1 && fsize != 9934413)
	    {
...
//        char msgbuf[80];
	...
	while(1)
	{
	    if(serialnum == 960416 || serialnum == 960313)	// FOR PSP: REG v1.0 - v1.31 & SHR v1.1
    	    	goto load_the_game;
	    else						// FOR PSP: UNKNOWN IWAD DETECTED
	    {
