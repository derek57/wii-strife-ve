...
*/
    ...
    // haleyjd 08/30/10: [STRIFE] No itemcount		// ADDED FOR PSP-STATS
...
*/
    ...
    if(!item)
    {
        ...
        case MT_SHADOWGUARD:
            ...
            break;
#ifdef SHAREWARE
	if(STRIFE_1_1_SHAREWARE)	// FOR PSP: IN THE SHAREWARE, THE GREEN ACOLYTE (MT_GUARD5)
	{				// DROPS A PASSCARD IN THE FIRST ROOM OF MAP 1 WHEN KILLED.
    	    case MT_GUARD5:		//
            	item = MT_KEY_TRAVEL;	// IT'S REQUIRED CAUSE YOU NEED THIS PASSCARD TO BE ABLE TO
            	break;			// OPEN UP THE FIRST DOOR OF THE FIRST ROOM IN WHICH THE
	}				// PLAYER GETS SPAWNED IN AFTER STARTING A NEW GAME!!!
#endif
        case MT_SPECTRE_B:
            item = MT_SIGIL_B;
            break;
