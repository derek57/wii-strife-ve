...
int		totalitems;		// ADDED FOR PSP-STATS
...
#define MAXPLMOVE	/*(forwardmove[1])*/	0x32
...
/*
#ifdef SHAREWARE
extern int		passcard_was_dropped;
...
int	joybdrop = 12;
int	joybkeys = 14;		// key_invkey	=	(k)	=	(16384)	=	(Y)
int	joybinvpop = 0;		// key_invpop	=	(z)	=	(1)	=	(A)
int	joybmission = 13;	// key_mission	=	(w)	=	(8192)	=	(X)
int     joybinvleft = 11;
int     joybinvright = 10;
int     joybinvuse = 2;
int     joybfire = 1;
...
{ 
//    int		i; 
    boolean	strafe/*, use*/;		// "USE" ADDED FOR PSP
//    boolean	bstrafe; 		// NOT NEEDED FOR PSP
//    int		speed/* = 0*/;
//    int		tspeed; 
    int		forward;
    int		side;
...
*/
    ...
    // ...CLOSED DOOR OR WALL AND A ROOM IS BEHIND THAT...
    // ...DOOR / WALL, THEN THE ITEM GETS PASSED BEHIND THAT...
    // ...DOOR / WALL)

    // villsa [STRIFE] inventory drop key			// CHANGED FOR PSP (SAVES A KEY)
    if(joybuttons[joybdrop] && !paused)			// IF THE KEY FOR DROP IS PRESSED...
    {								// ...AND THE GAME IS NOT PAUSED:
        player_t* player = &players[consoleplayer];		// - SET (DEFINE) THE PLAYER

        if(player->numinventory > 0)				// IF THERE'S AN ITEM THAT WE CAN DROP:
        {
	    cmd->buttons2 |= BT2_INVDROP;			// - DROP THAT ITEM
            cmd->inventory = player->inventory[player->inventorycursor].sprite; // - RESET INV. CURSOR

	    joybuttons[joybdrop] = false;					// - RESET THE KEYPRESS
        }
    }
/*
...
*/
    ...
    if (players[consoleplayer].health <= 15)		// COMPLETELY CHANGED FOR THE WII SOURCE PORT
    {
...
//            cmd->angleturn += angleturn[tspeed]; 
        ...
        if (joyirx < 0)     // calculate wii IR curve based on input
            cmd->angleturn -= turnspd * joyirx * 1.23;// FIX FOR THE WII: IN A SOMEWHAT STRANGE CASE,
    } 							// TURNING LEFT IS SLOWER THAN TURNING RIGHT???
							// (THIS IS ONLY THE CASE FOR STRIFE?! WTF?!)
...
*/
    ...
    else if (gamekeydown[key_fire] || joybuttons[joybfire]) 
...
*/
	...
	else if(data->btns_u & WPAD_CLASSIC_BUTTON_A)
	{
	    if(usergame)
	    {
		ev.type = ev_keyup;
	...
	if(data->btns_d & WPAD_CLASSIC_BUTTON_Y)
	{
	    if(usergame)
	    {
		ev.type = ev_keydown;
	...
	else if(data->btns_u & WPAD_CLASSIC_BUTTON_Y)
	{
	    if(usergame)
	    {
		ev.type = ev_keyup;
	...
	if(data->btns_d & WPAD_CLASSIC_BUTTON_X)
	{
	    if(usergame)
	    {
		ev.type = ev_keydown;
	...
	else if(data->btns_u & WPAD_CLASSIC_BUTTON_X)
	{
	    if(usergame)
	    {
		ev.type = ev_keyup;
        ...
        { 
            ...
            { 
                ...
                dclickstate2 = 0; 
						...
						// ADDED FOR PSP LOOK UP & LOOK DOWN
...
*/
	...
	adj *= mspeed;		// Speed up the X11 mlook a little.
...
/*
	...
	forward = 0;	// NEW ADDITION FOR PSP: DON'T MOVE THE PLAYER IF CHANGING VERTICAL VIEW ANGLE
*/
    }
/*
...
{ 
    ...
    joyxmove = joyymove = joystrafemove = joyirx = joyiry = 0;
    mousex = mousey = 0;
...
#ifdef SHAREWARE
    if(STRIFE_1_1_SHAREWARE)			// FOR PSP: RESET PASSCARD DROP STATUS FOR SHAREWARE
	passcard_was_dropped = 0;
#endif
*/
...
/*							// FOR PSP: DEACTIVATED HERE AND MOVED TO...
        if (AM_Responder (ev)) 				// ...D_MAIN.C (FUNCTION D_PROCESSEVENTS)...
            return true;	// automap ate it 	// ...BECAUSE OF AUTOMAP ZOOM LOOPING EVEN...
*/							// ...IF THE KEY HAS ALMOST BEING RELEASED
    } 
...
*/
        mousex = ev->data2*(mouseSensitivity+5)/7; 
        mousey = ev->data3*(mouseSensitivity+5)/7; 
...
{ 
    int         i;
//    int         buf; 			// PSP-GCC v4.8.2 compiler warning
    ticcmd_t*   cmd;
...
//    buf = (gametic/ticdup)%BACKUPTICS;  			// PSP-GCC v4.8.2 compiler warning
    ...
    for (i=0 ; i<MAXPLAYERS ; i++)
    {
        ...
        { 
            ...
            { 
                ...
                case BTS_PAUSE: 
//                    paused ^= 1; 		// ON PSP THIS WOULD DOUBLE-MAP AN INPUT KEY
                    if (paused) 
                        S_PauseSound (); 
                    else 
                        S_ResumeSound (); 
...
{ 
    ...
    int		itemcount;				// ADDED FOR PSP-STATS
    ...
    p->itemcount             = itemcount;	     // ADDED FOR PSP-STATS
...
//    charname = "PSP-Strife";	// FOR PSP: workaround for name (it works but it's way too less info)
