...
*/
    // villsa [STRIFE]
    if(ev->data1 == 'z' || ev->data1 == 'k' || ev->data1 == 'w')
    {
        if(ev->data1 == 'k')
        {
            st_showobjective = false;
...
*/
                st_keypage = -1;
								// DISABLED (FIX) FOR WII: WHY DOES
//                st_popupdisplaytics = ev->data2 ^ 'w';	// THIS CAUSE "TROUBLE" WHEN MISSION
								// OBJECTIVES ARE BEING CALLED???
...
//    byte *drawpos = I_VideoBuffer + y * SCREENWIDTH + x;			// CHANGED FOR HIRES
    byte *drawpos = I_VideoBuffer + (y << hires) * SCREENWIDTH + (x << hires);	// CHANGED FOR HIRES
    int i = 0;

//    while(i < len)								// CHANGED FOR HIRES
    while(i < (len << hires))							// CHANGED FOR HIRES
    {
	if (hires)								// ADDED FOR HIRES
	    *(drawpos + SCREENWIDTH) = putcolor;				// ADDED FOR HIRES
