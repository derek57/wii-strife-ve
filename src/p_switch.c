...
switchlist_t alphSwitchListFull[] =				// FOR PSP: (SHAREWARE RESTRICTIONS)
{
...
#ifdef SHAREWARE
switchlist_t alphSwitchListDemo[] =			// FOR PSP: (SHAREWARE RESTRICTIONS)
{
    { "GLASS01",    "GLASS02",      1,  sfx_bglass  },
    { "GLASS03",    "GLASS04",      1,  sfx_bglass  },
    { "GLASS05",    "GLASS06",      1,  sfx_bglass  },
...
static switchlist_t *alphSwitchList = NULL;	// FOR PSP: (SHAREWARE RESTRICTIONS)
#else
static switchlist_t *alphSwitchList = alphSwitchListFull;	// FOR PSP: (SHAREWARE RESTRICTIONS)
#endif
...
void P_InitSwitchList(void)
{
	...
	if ( gamemode == commercial )
	    episode = 3;*/
#ifdef SHAREWARE
    if (alphSwitchList == NULL)			// FOR PSP: (SHAREWARE RESTRICTIONS)
    {
	alphSwitchList = (STRIFE_1_0_SHAREWARE ||
...
boolean P_UseSpecialLine(mobj_t* thing, line_t* line, int side)
{
    ...
    case 171: // [STRIFE] TODO: which key is it?
        ...
        if(EV_DoLockedDoor(line, opend, thing))
            P_ChangeSwitchTexture(line, 0);
        break;
