...
//	I_QuitSerialFail();
    ...
    else
    {
        ...
        fsize = ftell(file_strife1_wad);	// take position of file pointer size variable
...
//	    printf(" Unable to open VOICES.WAD");
    	...
    	else
	{
	    fseek(file_voices_wad, 0, 2);		// file pointer at the end of file
    	    voices_wad_exists = 1;
	    fsizev = ftell(file_voices_wad);	// take position of file pointer size variable
	...
	{
	    fseek(fprw, 0, 2);		// file pointer at the end of file
	    fsizev = ftell(fprw);	// take a position of file pointer un size variable
