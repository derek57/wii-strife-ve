...
//    location_t *newLoc = M_Calloc(1, sizeof(location_t));
...
//    buf     = M_Calloc(lumplen + 1, sizeof(char));
...
static void P_parseLocScript(char *input)
{
    ...
    QStrFree(&qs);
    Z_Free(ls.input, "P_parseLocScript");
}
...
void P_SetLocationsFromFile(const char *filepath)
{
    ...
    Z_Free(srcfile, "P_SetLocationsFromFile");
}
...
void P_WriteActiveLocations(const char *filepath)
{
    ...
    Z_Free(dstfile, "P_WriteActiveLocations");
}
