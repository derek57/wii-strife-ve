...
//static int 	finit_height = SCREENHEIGHT - 32;			// CHANGED FOR HIRES
static int	finit_height = SCREENHEIGHT - (32 << hires);		// CHANGED FOR HIRES
...
{
    int i;
//    char namebuf[9];				// unused cause of fix
  
    for (i=0;i<10;i++)
    {
//        sprintf(namebuf, 9, "PLMNUM%d", i);	// CHANGE FOR PSP: throws compiler warning (is fixed)
//        W_ReleaseLumpName(namebuf);		// unused cause of fix
        ...
        if(markpoints[i].x != -1)
        {
  	    ...
  	    if (automapactive & am_rotate)
            ...
            if(fx >= f_x && fx <= f_w - w && fy >= f_y && fy <= f_h - h)
            {
                // villsa [STRIFE]
                if(i >= mapmarknum)
//                    V_DrawPatch(fx, fy, marknums[i]);				// CHANGED FOR HIRES
		    V_DrawPatch(fx >> hires, fy >> hires, marknums[i]);		// CHANGED FOR HIRES
            }
        }
    }
...
void AM_Drawer (void)
{
    ...
    // villsa [STRIFE] USUSALLY it's not used
    if(drawgrid == 1)
